...
import { Spacer, Display, Code, Dot, Image, Snippet } from '@geist-ui/core'
...
则需要发布一个新的版本来修复这个错误。这只是约定，在缺乏自动化时我们需要对每个操作负责。
...
通常是运行一个服务端来监听 Git 事件，在合适的时机向你发起一个 PR 升级版本。(有关 robot 你可以关注我的项目 [cobot.gitlab](https://github.com/unix/cobot)）
...
对于本地升级版本的方式你可以试试 [done](https://github.com/unix/done)，
...
- 发布版本时可借助 conventional-changelog-cli 自动生成所有的版本日志
...
在发布源码的基础上使用 [release-drafter](https://probot.github.io/apps/release-drafter/) 等 Git Bot，
...
- 升级版本后需要本地执行一次 Build 脚本，或是在 `scripts.prepublish` 中指定 Build 语句。
...
相关的脚本配置可以参考我的项目 [Geist](https://github.com/geist-org/geist-ui/blob/master/.github/workflows/publish.yml)。
